...
- [x] Task 1: SYS-003 -- Substituir JWT customizada por PyJWT: adicionar dependencia, implementar encode/decode com audience, issuer, expiration, e key rotation support
- [x] Task 2: SYS-003 -- Configurar JWK (JSON Web Key) para key management; rotacao de chaves sem downtime
- [x] Task 3: DB-005 + DB-014 -- Substituir `==` por `hmac.compare_digest` nos 2 pontos de comparacao de API key: middleware (DB-005) e /auth/token (DB-014, main.py linha 317)
- [x] Task 4: SYS-004 -- Restringir CORS allow_headers para lista explicita: Content-Type, Authorization, X-API-Key
- [x] Task 5: SYS-005 -- Adicionar middleware para Content-Security-Policy header com politica restritiva
- [x] Task 6: SYS-005 -- Adicionar Strict-Transport-Security header (HSTS) com max-age adequado
- [x] Task 7: SYS-007 -- Adicionar safeguard: quando API_KEY ou JWT_SECRET nao definidos em producao (NODE_ENV=production), falhar com erro explicito ao inves de bypass silencioso
- [x] Task 8: SYS-007 -- Log warning em dev mode quando auth bypass ativo: "Auth bypass active -- development mode only"

## Criterios de Aceite

- [x] JWT usa PyJWT com audience, issuer, e expiration validados
- [x] Key rotation funcional sem downtime
- [x] `hmac.compare_digest` usado em AMBOS os pontos de auth (validado por teste unitario)
- [x] CORS rejeita headers nao listados explicitamente
- [x] Content-Security-Policy header presente em todas as respostas
- [x] Strict-Transport-Security header presente com max-age >= 31536000 (1 ano)
- [x] Aplicacao falha ao iniciar em producao se API_KEY ou JWT_SECRET nao definidos
- [x] Log warning emitido em dev mode quando auth bypass ativo
...
- [x] Security headers validados em ambiente de staging
